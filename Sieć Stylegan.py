...
from datagen2 import dataGenerator, printProgressBar

im_size = 256
latent_size = 512
BATCH_SIZE = 12
directory = "oct_dataset_resized"
...
class GAN(object):
    def __init__(self, steps = 1, lr = 0.00001, decay = 0.000001):
    ...
    def generator(self):
        ...
        x = Conv2D(filters = 1, kernel_size = 1, padding = 'same', kernel_initializer = 'he_normal')(x)
    ...
    def GenModelA(self):
        ...
        for i in range(n_layers):
            ...
            yy = lambda x: x * trunc; style[-1] = yy(style[-1])
...
class StyleGAN(object):
    ...
    def train(self):
        ...
        if self.GAN.steps % 100 == 0 and not self.silent:
            ...
            if self.GAN.steps % 10000 == 0 or (self.GAN.steps % 100 == 0 and self.GAN.steps < 1000):
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        x.save("Results1/i"+str(num)+"-ema.png")
        ...
        x.save("Results1/i"+str(num)+"-mr.png")

    def saveModel(self, model, name, num):
        json = model.to_json()
        with open("Models1/"+name+".json", "w") as json_file:
            json_file.write(json)

        model.save_weights("Models1/"+name+"_"+str(num)+".h5")

    def loadModel(self, name, num):
        file = open("Models1/"+name+".json", 'r')
        json = file.read()
        file.close()

        mod = model_from_json(json)
        mod.load_weights("Models1/"+name+"_"+str(num)+".h5")
    ...
    def createFrame(self, list1, list2, nIm, alpha = 0, fnum = 0):
        ...
        x.save("Results1/Frames/frame-"+str(fnum)+".png")
